...
void RCC_Config()
{
  ...
  if ( RCC_WaitForHSEStartUp() == SUCCESS )      //等待外部时钟启动  25MHz
  {
    ...
    RCC_PCLK1Config(RCC_HCLK_Div4);              //PCLK1(APB1)时钟为HCLK时钟4分频 42MHz;TIM2,3,4,5,6,7,12,13,14为84MHz
    RCC_PCLK2Config(RCC_HCLK_Div2);              //PCLK2(APB2)时钟为HCLK时钟2分频 84Mhz;TIM1,8,9,10,11为168MHz
